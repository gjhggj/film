...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
	User user = (User)request.getSession().getAttribute("user");//从会话中获取user对象。
	if(user == null){
		response.sendRedirect("../mainPage.jsp");//如果用户未登录，重定向到主页面。
	}else{
	    if(user.getUser_role() != 1){
	        response.sendRedirect("../mainPage.jsp");//如果用户不是管理员（假设1代表管理员角色），也重定向到主页面。
	    }
	}
%>
<!DOCTYPE html>
...
<body>
    ...
    <script>
        var clientHeight = document.documentElement.clientHeight;//获取当前页面的高度，用于后续设置弹出框的位置。
        var AddCinemaHtml;
        var type = [];
        ...
        function initCinema(){
            ...
            layui.use(['laypage', 'layer', 'table'], function(){
                ...
                table.render({
                    elem: '#cinema_table_id'//选择要渲染表格的HTML元素。
                    ,url: url + '/cinema'//从这个URL获取数据。
                    ,method: 'get'//使用GET方法请求数据。
                   	,where: {
            	      method: 'findAllCinema'//设置请求参数。
            	    }
                    ,toolbar: '#cinematoolbar'
                    ,defaultToolbar: []
                    ,title: '影院列表'
                    ,cols: [[//定义表格的列，包括影院ID、名称、地址和操作列。
                ...
                //监听工具条
                table.on('tool(CinemaTable)', function(obj){//监听表格工具栏事件。
                    var data = obj.data;
                   if(obj.event === 'edit'){
